TiThTeTsGdirectGdiffuse <- function(Dat)
  {
    ...
    model$addSystem(dTi ~ ( 1/(Ci*Ris)*(Ts-Ti) + 1/(Ci*Rih)*(Th-Ti) + 1/(Ci*Rie)*(Te-Ti) + gAdirect/Ci*Gdirect + gAdiffuse/Ci*Gdiffuse )*dt + exp(p22)*dw2 )
    ...
    model$setParameter( gAdirect = c(init=20  ,lb=0.01  ,ub=300))
    model$setParameter( gAdiffuse = c(init=5  ,lb=0.01  ,ub=300))
