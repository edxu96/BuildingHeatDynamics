...
cal_ll_o$d <- function(x, y_i, sigma) {
  (pnorm(y_i[2], mean = x, sd = sigma) -
  pnorm(y_i[1], mean = x, sd = sigma))^9 *
  dnorm(y_i[2], mean = x, sd = sigma) *
  dnorm(y_i[1], mean = x, sd = sigma)
}

cal_ll_o$e <- function(x, y_i, sigma) {
  (1 - pnorm(y_i[2], mean = x, sd = sigma))^9 *
  dnorm(y_i[1], mean = x, sd = sigma) *
  dnorm(y_i[2], mean = x, sd = sigma)
}
...
ll_o$a <- sapply(theta_k, lla.f, y_i = y_i, sigma = sigma)
plot(theta_k, ll$a / max(ll$a), type = "l")

ll_o$b <- sapply(theta_k, lla.f, y_i = mean(y_i), sigma = sigma / sqrt(n))
lines(theta_k, ll$b / max(ll$b), col = 2, type = "l")

ll_o$c <- sapply(theta_k, llc.f, y_i = median(y_i), sigma = sigma)
