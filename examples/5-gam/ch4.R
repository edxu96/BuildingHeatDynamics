## ch4: Generalized Additive Model, GAM_Simon-Wood ----
...
plot(size, wear, xlab = "Engine capacity", ylab = "Wear index", bty = "n")
...
plot(size, wear, bty = "n") ## plot data
lines(s, Xp %*% coef(b), col = 2) ## overlay estimated f
...
plot(size, wear, bty = "n") ## plot data
Xp <- tf.X(s, sj) ## prediction matrix
lines(s, Xp %*% coef(b), col = 2) ## plot the smooth
...
plot(rho, V,
  type = "l", xlab = expression(log(lambda)),
  main = "GCV score"
)
...
am.fit <- function(y, x, v, sp, k = 10) {
  ...
  ## return fitted model...
  list(
    b = coef(b), Vb = Vb, edf = trA, gcv = gcv, fitted = fitted(b)[1:n],
...
am.plot <- function(fit, xlab, ylab) {
  ...
  for (i in 1:2) {
    ...
    plot(x, fv,
      type = "l", ylim = range(c(ul, ll)), xlab = xlab[i],
      ylab = ylab[i]
    )
    lines(x, ul, lty = 2)
    lines(x, ll, lty = 2)
