...
k <- 24
X <- data.frame(t = li_data$t, Pw = li_data$Pw, Ws = lag_vector(li_data$Wsnwp[, pst("k", k)], k))
...
obj <- function(prm, frml, li_data, k, ieval = 1:nrow(li_data)) {
  print(prm)
...
result <- optimize(obj, lower = 0.95, upper = 1, frml = frml, li_data = X[itrain, ], k = k, ieval = ieval)
result$minimum
...
## Optimize
result <- optim(
  h, obj_kernel, lower = 2, upper = 8, frml = frml, li_data = X[itrain, ],
  ieval = ieval, k = k
  )
result
