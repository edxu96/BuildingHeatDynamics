...
## Use the function defined in the file "functions/sim_prbs.R", which generates a PRBS signal
...
## ----------------------------------------------------------------
## Generate the PRBS signals for the PRBS1 experiment, where a single signal
## controls all heaters in the building
...
## Multiple PRBS signals are simply generated lagging the signals, such that
## they do not begin at the same time point
x1 <- sim_prbs(n = 6, initReg = 666, lambda = 4)
n <- length(x1)
...
x3 <- rep(x3, 2)
## The long period
x1.long <- sim_prbs(n = 5, initReg = 667, lambda = 36)
n <- length(x1.long)
