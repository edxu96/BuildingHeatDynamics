...
test_logLik_ratio(mod = ctsmr_TiTh, mod_s = ctsmr_Ti)

## ----------------------------------------------------------------
## PROCEED the forward selection
## The models which could be used in the next step are implemented in the
## following functions
...
test_logLik_ratio(ctsmr_TiThTe, ctsmr_TiTh)
...
cov <- diag(ctsmr_TiThTe$sd[i]) %*% ctsmr_TiThTe$corr[i, i] %*%
  diag(ctsmr_TiThTe$sd[i])

## Calculate the uncertainty of the HLC value with a linear approximation to
## the covariance The Jacobian, the derived of the HLC-value with respect to
## each estimate in ctsmr_TiThTe$xm[i]
J <- t(
  sapply(1:length(i), function(ii, x) {
    -1 / sum(x)^2
  }, x = ctsmr_TiThTe$xm[i])
)
