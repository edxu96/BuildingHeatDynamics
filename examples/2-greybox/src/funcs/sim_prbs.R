...
#' @param n is the length of the register
#' @param initReg just needs to be some initial value of 1,2,...
#'   it is the initial value of the registers and therefore only
#'   determines the start of the cycle
#' @param lambda is the length of the smallest period in
#'   which the signal can change, given in samples
sim_prbs <- function(n, initReg = 666, lambda = 1) {
  ##
  ## Check the input
  if (n < 2 | n > 11) {
    stop("n must be between 1 and 11")
  }
  ## Do init
  ...
  for (i in 1:N) {
    ...
    if (n <= 4 | n == 6) {
      ...
      reg[n + 1] <- as.raw(
        sum(as.integer(c(reg[1], reg[5], reg[6], reg[7]))) %% 2
        )
    }
    else if (n == 9) {
      reg[n + 1] <- xor(reg[1], reg[5])
    }
