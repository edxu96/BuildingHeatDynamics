sdeTiTw2_GinTw_sigmalevels <- function(X, rerun = FALSE){
    ...
    model$addSystem(dTi  ~ ( 1/(Ci*Riw)*(Tw1-Ti) + 1/Ci*Qi + gA*p/Ci*Gv )*dt + (1+(stepQi*sigmalevel))*exp(p11)*dw1)
    model$addSystem(dTw1 ~ ( 1/(Cw1*Riw)*(Ti-Tw1) + 1/(Cw1*Rww)*(Tw2-Tw1) + gA*(1-p)/Cw1*Gv )*dt + (1+(stepQi*sigmalevel))*exp(p22)*dw2) # gA*(1-p)/Cw*Gv
