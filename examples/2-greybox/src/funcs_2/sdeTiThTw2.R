sdeTiThTw2 <- function(X, rerun = FALSE){
    ...
    model$addSystem(dTi  ~ ( 1/(Ci*Riw)*(Tw1-Ti) + p*gA/Ci*Gv + 1/(Ci*Rih)*(Th-Ti) )*dt + exp(p11)*dw1)
    model$addSystem(dTh  ~ ( 1/(Ch*Rih)*(Ti-Th) + 1/Ch*Qi )*dt + exp(p22)*dw2)
    model$addSystem(dTw1 ~ ( 1/(Cw1*Riw)*(Ti-Tw1) + 1/(Cw1*Rww)*(Tw2-Tw1) + (1-p)*gA/Cw1*Gv )*dt + exp(p33)*dw3) 
    ...
    model$setParameter(  gA = c(init=0.1 ,lb=0.01 ,ub=1) )
