...
plot_series <- function(
    X, prefix, gridTs, xnm, spaceForLegend = 0.25, cex.legend = 1, colmap = NA,
    ylim = NA, main = "", ...
    ) {
  ##  browser()
  iseq <- integer(0)
  ...
  ## Limits on y-axis
  if (is.na(ylim[1])) {
    ylim <- range(X[, iseq], na.rm = TRUE)
  }
  ##
  plot(
    X[, xnm], X[, xnm], type = "n", xlim = c(min(X[, xnm]),
    max(X[, xnm]) + diff(range(X[, xnm])) * spaceForLegend), ylim = ylim,
  ...
  rng <- do.call("rbind", lapply(1:length(iseq), function(i) {
    paste(format(range(X[, iseq[i]], na.rm = TRUE), digits = 2), collapse = " to ")
  }))
