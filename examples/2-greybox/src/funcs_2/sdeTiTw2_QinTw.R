sdeTiTw2_QinTw <- function(X, rerun = FALSE){
    ...
    model$addSystem(dTi  ~ ( 1/(Ci*Riw)*(Tw1-Ti) + gA*p/Ci*Gv )*dt + exp(p11)*dw1)
    model$addSystem(dTw1 ~ ( 1/(Cw1*Riw)*(Ti-Tw1) + 1/Cw1*Qi + 1/(Cw1*Rww)*(Tw2-Tw1) + gA*(1-p)/Cw1*Gv )*dt + exp(p22)*dw2) # gA*(1-p)/Cw*Gv
